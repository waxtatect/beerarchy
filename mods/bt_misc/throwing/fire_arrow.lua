...
THROWING_ARROW_ENTITY.on_step = function(self, dtime)
		...
		for k, obj in pairs(objs) do
			if obj:get_luaentity() then
				if obj:get_luaentity().name ~= "throwing:arrow_fire_entity" and obj:get_luaentity().name ~= "__builtin:item" then
					...
					if name then
						extra_damage = ranking.playerXP[name] + math.random(-3, 3)
	...
	if self.lastpos.x then
		if node.name ~= "air" and node.name ~= "throwing:light" then
			if not minetest.is_protected(self.lastpos, name) then
				minetest.set_node(self.lastpos, {name = "fire:basic_flame"})
			end
			throwing.playerArrows[self.object] = nil
			self.object:remove()
		end
		if not vector.equals(vector.floor(vector.add(self.lastpos, 0.5)), vector.floor(vector.add(pos, 0.5))) then
			if minetest.get_node(self.lastpos).name == "throwing:light" then
				minetest.remove_node(self.lastpos)
			end
			if minetest.get_node(pos).name == "air" and not minetest.is_protected(pos, name) then
