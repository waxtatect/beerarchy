...
local main_channel_owner = minetest.settings:get("name")		-- The owner of the main channel, usually ADMIN
...
minetest.register_on_joinplayer(function(player)
	local str = player:get_meta():get_string("beerchat:channels")
	local name = player:get_player_name()
	if str and str ~= "" then
		playersChannels[name] = minetest.parse_json(str) or {}
	else
		playersChannels[name] = {}
		playersChannels[name][main_channel_name] = "joined"
		player:get_meta():set_string("beerchat:channels", minetest.write_json(playersChannels[name]))
	end

	local current_channel = player:get_meta():get_string("beerchat:current_channel")
	if current_channel and current_channel ~= "" then
		currentPlayerChannel[name] = current_channel
	else
...
local create_channel = {
	...
	func = function(lname, param)
		...
		minetest.get_player_by_name(lowner):get_meta():set_string("beerchat:channels", minetest.write_json(playersChannels[lowner]))
		if enable_sounds then
			minetest.sound_play(channel_management_sound, {to_player = lowner, gain = 1.0}, true)
		end
		chat_send_player(lowner, format_message(channel_created_string, {channel_name = lchannel_name}))
...
local delete_channel = {
	...
	func = function(name, param)
		...
		minetest.get_player_by_name(name):get_meta():set_string("beerchat:channels", minetest.write_json(playersChannels[name]))

		if enable_sounds then
			minetest.sound_play(channel_management_sound, {to_player = name, gain = 1.0}, true)
		end

		chat_send_player(name, format_message(channel_deleted_string, {channel_name = param, color = color}))
...
local my_channels = {
	...
	func = function(name, param)
		if not param or param == "" then
			...
			chat_send_player(name, dump2(playersChannels[name]))
		else
			if playersChannels[name][param] then
				if enable_sounds then
					minetest.sound_play(channel_management_sound, {to_player = name, gain = 1.0}, true)
				end
				chat_send_player(name, dump2(channels[param]))
			else
				chat_send_player(name, "ERROR: Channel not in your channel list")
				return false
			end
		end
		return true
...
local join_channel = {
	...
	func = function(name, param)
		...
		minetest.get_player_by_name(name):get_meta():set_string("beerchat:channels", minetest.write_json(playersChannels[name]))
		if enable_sounds then
			minetest.sound_play(join_channel_sound, {to_player = name, gain = 1.0}, true)
		end
		chat_send_player(name, format_message(channel_joined_string, {channel_name = channel_name}))
...
local leave_channel = {
	...
	func = function(name, param)
		...
		if enable_sounds then
			minetest.sound_play(leave_channel_sound, {to_player = name, gain = 1.0}, true)
		end
		if not channels[channel_name] then
			chat_send_player(name, format_message(channel_already_deleted_string, {channel_name = channel_name}))
		else
			chat_send_player(name, format_message(channel_left_string, {channel_name = channel_name}))
		end
...
local invite_channel = {
	...
	func = function(name, param)
		...
		if not player then
			...
			if player:get_meta():get_string("beerchat:muted:" .. name) == "" then
				if enable_sounds then
					minetest.sound_play(channel_invite_sound, {to_player = player_name, gain = 1.0}, true)
				end
				-- Sending the message
				chat_send_player(player_name, format_message(channel_invitation_string, {channel_name = channel_name, from_player = name}))
			end
			if enable_sounds then
				minetest.sound_play(channel_invite_sound, {to_player = name, gain = 1.0}, true)
			end
			chat_send_player(name, format_message(channel_invited_string, {channel_name = channel_name, to_player = player_name}))
		end
...
local mute_player = {
	...
	func = function(name, param)
		...
		minetest.get_player_by_name(name):get_meta():get_string("beerchat:muted:" .. param, "true")
		chat_send_player(name, "Muted player " .. param)
...
local unmute_player = {
	...
	func = function(name, param)
		...
		chat_send_player(name, "Unmuted player " .. param)
...
minetest.register_on_chat_message(function(name, message)
	...
	if players and msg then
		if msg == "" then
			chat_send_player(name, "Please enter the private message you would like to send")
		else
			...
			if players and players ~= "" then
				...
				for target in string.gmatch(","..players..",", ",([^,]+),") do
					local pl_target = minetest.get_player_by_name(target)
					-- Checking if the target exists
					if not pl_target then
						chat_send_player(name, ""..target.." is not online")
					else
						if pl_target:get_meta():get_string("beerchat:muted:" .. name) == "" then
							if target ~= name then
								-- Sending the message
								chat_send_player(target, format_message(private_message_string, {from_player = name, message = msg}))
								if enable_sounds then
									minetest.sound_play(private_message_sound, {to_player = target, gain = 1.0}, true)
								end
							else
								chat_send_player(target, format_message(self_message_string, {from_player = name, message = msg}))
								if enable_sounds then
									minetest.sound_play(self_message_sound, {to_player = target, gain = 1.0}, true)
								end
							end
						end
						atleastonesent = true
				...
				if atleastonesent then
					...
					if (successplayers ~= name) then
						chat_send_player(name, format_message(private_message_sent_string, {to_player = successplayers, message = msg}))
					end
				end
			else
				chat_send_player(name, "You have not sent private messages to anyone yet, please specify player names to send message to")
			end
		end
		return true
	end
...
local msg_override = {
	...
	func = function(name, param)
		...
		if players and msg then
			if players == "" then
				chat_send_player(name, "ERROR: Please enter the private message you would like to send")
				return false
			elseif msg == "" then
				chat_send_player(name, "ERROR: Please enter the private message you would like to send")
				return false
...
local me_override = {
	...
	func = function(name, param)
		...
		if not channels[channel_name] then
			chat_send_player(name, "Channel "..channel_name.." does not exist")
		elseif msg == "" then
			chat_send_player(name, "Please enter the message you would like to send to the channel")
		elseif not playersChannels[name][channel_name] then
			chat_send_player(name, "You need to join this channel in order to be able to send messages to it")
		else
			for _, player in ipairs(minetest.get_connected_players()) do
				...
				if playersChannels[target][channel_name] then
					if player:get_meta():get_string("beerchat:muted:" .. name) == "" then
						chat_send_player(target, format_message(me_message_string, {channel_name = channel_name, from_player = name, message = msg}))
					end
				end
			end
		end
...
minetest.register_on_chat_message(function(name, message)
	...
	if channel_name and msg then
		if not channels[channel_name] then
			chat_send_player(name, "Channel "..channel_name.." does not exist. Make sure the channel still "..
					...
					if playersChannels[target][channel_name] then
						if player:get_meta():get_string("beerchat:muted:" .. name) == "" then
							if channel_name == main_channel_name then
								chat_send_player(target, format_message(main_channel_message_string, {channel_name = channel_name, from_player = name, message = msg}))
							else
								chat_send_player(target, format_message(channel_message_string, {channel_name = channel_name, from_player = name, message = msg}))
								if enable_sounds then
									minetest.sound_play(channel_message_sound, {to_player = target, gain = 1.0}, true)
								end
							end
						end
					end
		...
		if channel_name then
			if not channels[channel_name] then
				chat_send_player(name, "Channel "..channel_name.." does not exist")
			elseif not playersChannels[name][channel_name] then
				chat_send_player(name, "You need to join this channel in order to be able to switch to it")
			else
				...
				if channel_name == main_channel_name then
					chat_send_player(name, "Switched to channel "..channel_name..", messages will now be sent to this channel")
				else
					chat_send_player(name, "Switched to channel "..channel_name..", messages will now be sent to this channel. To switch back "..
...
minetest.register_on_chat_message(function(name, message)
	...
	if dollar == "$" then
		...
		if radius > whisper_max_range then
			chat_send_player(name, "You cannot whisper outside of a radius of "..whisper_max_range.." blocks")
		elseif msg == "" then
			chat_send_player(name, "Please enter the message you would like to whisper to nearby players")
		else
			local pl_pos = minetest.get_player_by_name(name):get_pos()
			...
			for _, player in ipairs(all_objects) do
				if player:is_player() then
					...
					if playersChannels[target][main_channel_name] then
						if player:get_meta():get_string("beerchat:muted:" .. name) == "" then
							chat_send_player(target, format_message(whisper_string, {
...
minetest.register_on_chat_message(function(name, message)
	...
	if not channels[channel_name] then
		chat_send_player(name, "Channel "..channel_name.." does not exist, switching back to "..main_channel_name..". Please resend your message")
		currentPlayerChannel[name] = main_channel_name
		minetest.get_player_by_name(name):get_meta():set_string("beerchat:current_channel", main_channel_name)
		return true
	end
	...
	elseif not playersChannels[name][channel_name] then
		...
		for _, player in ipairs(minetest.get_connected_players()) do
			...
			if playersChannels[target][channel_name] then
				if player:get_meta():get_string("beerchat:muted:" .. name) == "" then
					chat_send_player(target, format_message(main_channel_message_string, {channel_name = channel_name, from_player = name, message = message}))
