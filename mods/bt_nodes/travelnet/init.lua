...
--]]
-- minetest.safe_file_write: required to save the travelnet data properly in all cases (version >= 0.4.17)

if not minetest.get_translator then
	error("[Mod travelnet] Your Minetest version is no longer supported. (version < 5.0.0)")
end

local use_locks = minetest.global_exists("locks")
...
travelnet.save_data = function()
	...
	if not success then
		print(("[Mod travelnet] Error: Savefile '%s' could not be written.")
...
travelnet.restore_data = function()
	...
	if not file then
		print(("[Mod travelnet] Error: Savefile '%s' not found.")
			:format(travelnet.mod_data_path))
		return
	end
	...
	if not travelnet.targets then
		...
		print(("[Mod travelnet] Error: Savefile '%s' is damaged. Saved the backup as '%s'.")
...
travelnet.check_if_trying_to_dig = function(puncher, node)
	...
	if creative and creative.is_enabled_for(puncher:get_player_name())
		-- and (not puncher:get_wielded_item()) or puncher:get_wielded_item():get_name() ~= "default:pick_diamond"
	then
		return false
	end
...
travelnet.show_message = function(pos, player_name, title, message)
	...
	minetest.show_formspec(player_name, "travelnet:show",
		"size[8,3]"..
		"label[3,0;"..F(S(title or "Error")).."]"..
		"textlist[0,0.5;8,1.5;;"..F(message or S("- nothing -"))..";]"..
		"button_exit[3.5,2.5;1.0,0.5;back;"..F(S("Back")).."]"..
		"button_exit[6.8,2.5;1.0,0.5;station_exit;"..F(S("Exit")).."]"..
...
travelnet.show_current_formspec = function(pos, meta, player_name)
	...
	-- show the formspec manually, we need to supply the position of the travelnet box
	minetest.show_formspec(player_name, "travelnet:show",
...
travelnet.form_input_handler = function(player, formname, fields)
	if formname == "travelnet:show" and fields and fields.pos2str then
		...
		if use_locks and (fields.locks_config or fields.locks_authorize) then
...
travelnet.reset_formspec = function(meta)
	...
	meta:set_string("formspec",
		"size[10,6.0]"..
		"label[2.0,0.0;--> "..F(S("Configure this travelnet station")).." <--]"..
		"button_exit[8.0,0.0;2.2,0.7;station_dig;"..F(S("Remove station")).."]"..
		"field[0.3,1.2;9,0.9;station_name;"..F(S("Name of this station"))..":;"..F(station_name or "").."]"..
		"label[0.3,1.5;"..F(S("How do you call this place here? Example: \"my first house\", \"mine\", \"shop\"...")).."]"..
		"field[0.3,2.8;9,0.9;station_network;"..F(S("Assign to Network:"))..";"..F(station_network or "").."]"..
		"label[0.3,3.1;"..F(S("You can have more than one network. If unsure, use \"%@1\"", tostring(station_network)))..".]"..
		"field[0.3,4.4;9,0.9;owner;"..F(S("Owned by:"))..";]"..
		"label[0.3,4.7;"..F(S("Unless you know what you are doing, leave this empty.")).."]"..
		"button_exit[1.3,5.3;1.7,0.7;station_help_setup;"..F(S("Help")).."]"..
		"button_exit[3.8,5.3;1.7,0.7;station_set;"..F(S("Save")).."]"..
		"button_exit[6.3,5.3;1.7,0.7;station_exit;"..F(S("Exit")).."]"
	)
end
...
travelnet.update_formspec = function(pos, puncher_name, fields)
	...
	if this_node and this_node.name == 'travelnet:elevator' then
		is_elevator = true
	end
...
--]]
		...
		travelnet.show_message(pos, puncher_name, "Error",
			S("Update failed! Resetting this box on the travelnet."))
		return
	end
	...
	then
		...
		minetest.chat_send_player(owner_name, S("Station '@1' has been reattached to the network '@2'.", station_name, station_network))
		travelnet.save_data()
	end
	...
	local zusatzstr = ""
	local trheight = "10"
	if use_locks and this_node and this_node.name == "locked_travelnet:travelnet" then
		zusatzstr = "field[0.3,11;6,0.7;locks_sent_lock_command;"..F(S("Locked travelnet. Type /help for help:"))..";]"..
			locks.get_authorize_button(10, "10.5")..locks.get_config_button(11, "10.5")
		trheight = "11.5"
	end
	local formspec = "size[12,"..trheight.."]"..
		"label[3.3,0.0;"..F(S("Travelnet-Box:")).."]".."label[6.3,0.0;"..F(S("Punch box to update target list.")).."]"..
		"label[0.3,0.4;"..F(S("Name of this station:")).."]".."label[6.3,0.4;"..F(station_name or "?").."]"..
		"label[0.3,0.8;"..F(S("Assigned to Network:")).."]" .."label[6.3,0.8;"..F(station_network or "?").."]"..
		"label[0.3,1.2;"..F(S("Owned by:")).."]"            .."label[6.3,1.2;"..F(owner_name or "?").."]"..
		"label[3.3,1.6;"..F(S("Click on target to travel there:")).."]"..
		-- "button_exit[5.3,0.3;8,0.8;do_update;"..F(S("Punch box to update destination list. Click on target to travel there."))..]"..
		zusatzstr
	local x = 0
	local y = 0
	local i = 0
	...
	if is_elevator then
		...
		table.sort(stations, function(a, b)
			return travelnet.targets[owner_name][station_network][a].timestamp < travelnet.targets[owner_name][station_network][b].timestamp
		end)
	end
	...
	-- Note: With elevators, only the "G"(ground) marking is actually moved
	if fields
	...
	then
		...
		if swap_with_pos < 1 then
			travelnet.show_message(pos, puncher_name, "Info",
				S("This station is already the first one on the list."))
			return
		elseif swap_with_pos > #stations then
			travelnet.show_message(pos, puncher_name, "Info",
				S("This station is already the last one on the list."))
			return
		else
...
--]]
	...
	formspec = formspec ..
		"label[7.45,1.6;"..F(S("Position in list:")).."]"..
		"button_exit[11.17,0.0;1.0,0.5;station_exit;"..F(S("Exit")).."]"..
		"button_exit[10.0,0.75;2.2,0.7;station_dig;"..F(S("Remove station")).."]"..
		"button[9.5,1.6;1.4,0.5;move_up;"..F(S("move up")).."]"..
		"button[10.8,1.6;1.4,0.5;move_down;"..F(S("move down")).."]"

	meta:set_string("formspec", formspec)

	meta:set_string("infotext",
		S("Station '@1' on travelnet '@2' (owned by @3) ready for usage. Right-click to travel, punch to update.",
			station_name, station_network, owner_name
	))
...
travelnet.add_target = function(station_name, network_name, pos, player_name, meta, owner_name)
	...
	if station_name == "" or (not station_name) then
		travelnet.show_message(pos, player_name, "Error",
			S("Please provide a name for this station."))
		return
	end
	...
	then
		travelnet.show_message(pos, player_name, "Error",
			S("You do not have the travelnet_attach priv which is required to attach your box to the network of someone else. Aborting."))
		return
	end
	...
	if anz+1 > travelnet.MAX_STATIONS_PER_NETWORK then
		travelnet.show_message(pos, player_name, "Error",
			S("Network '@1', already contains the maximum number (=@2) of allowed stations per network. Please choose a different/new network name.",
				network_name, travelnet.MAX_STATIONS_PER_NETWORK))
		return
	end
	...
	if meta then
		minetest.chat_send_player(player_name,
			S("Station '@1' has been added to the network '@2', which now consists of @3 station(s).",
				station_name, network_name, anz+1
		))
		...
		meta:set_string("formspec",
			"size[12,10]"..
			"field[0.3,0.6;6,0.7;station_name;"..F(S("Station:"))..";".. F(station_name or "") .."]"..
			"field[0.3,3.6;6,0.7;station_network;"..F(S("Network:"))..";".. F(station_network or "") .."]"
		)
...
travelnet.open_close_door = function(pos, player, mode)
	...
	if	   this_node.param2 == 0 then pos2.z = pos.z - 1
	elseif this_node.param2 == 1 then pos2.x = pos.x - 1
	elseif this_node.param2 == 2 then pos2.z = pos.z + 1
	elseif this_node.param2 == 3 then pos2.x = pos.x + 1
	end

	local door_node = minetest.get_node(pos2)
	if door_node and door_node.name ~= 'ignore' and door_node.name ~= 'air'
		and minetest.registered_nodes[door_node.name] and minetest.registered_nodes[door_node.name].on_rightclick
	then
...
travelnet.on_receive_fields = function(pos, formname, fields, player)
	...
	if fields.station_dig then
		...
		elseif node and node.name and node.name == "locked_travelnet:travelnet" then
			...
			minetest.chat_send_player(name, S("Error: Unknown node."))
			return
		end
		...
		if (not minetest.check_player_privs(name, {travelnet_remove=true}))
			...
			and owner ~= ""
		then
			minetest.chat_send_player(name, S("This @1 belongs to @2. You can't remove it.", S(description), owner))
			return
		end
	...
	then
		if owner_name and station_name and station_network then
			...
			minetest.chat_send_player(name, S("Error: There is something wrong with the configuration of this station."))
			print("ERROR: The travelnet at "..minetest.pos_to_string(pos).." has a configuration problem: "..
	...
	if (not owner_name)
		...
		or (not travelnet.targets[owner_name][station_network])
	then
		minetest.chat_send_player(name, S("Error: This travelnet is lacking data and/or improperly configured."))
	...
	if this_node and this_node.name == 'travelnet:elevator' then
		...
		local stations = travelnet.targets[owner_name][station_network]
		for k, _ in pairs(stations) do
			local station = stations[k].nr
			if minetest.get_translated_string(lang_code, station) == fields.target then -- station == fields.target
				fields.target = k
				break
			end
		end
	...
	if travelnet.travelnet_sound_enabled then
		minetest.sound_play("travelnet_transporter", {pos = pos, gain = 0.25, max_hear_distance = 10}, true)
		minetest.sound_play("travelnet_transporter_beep", {pos = pos, gain = 0.25, max_hear_distance = 10}, true)
		minetest.sound_play("travelnet_transporter", {to_player = name, gain = 0.25}, true)
	end
	...
	minetest.after(time, function(player, pos, target_pos)
		if player then
			...
			if node2 and node2.name ~= "ignore" and node2.name ~= "travelnet:travelnet" and node2.name ~= "travelnet:elevator"
				and node2.name ~= "locked_travelnet:travelnet" and node2.name ~= "travelnet:travelnet_private"
			then
				-- provide information necessary to identify the removed box
				local oldmetadata = {fields = {owner = owner_name, station_name = fields.target, station_network = station_network}}
...
travelnet.rotate_player = function(target_pos, player, tries)
	...
	if travelnet.travelnet_sound_enabled then
		minetest.sound_play("travelnet_transporter", {pos = target_pos, gain = 0.25, max_hear_distance = 10}, true)
	end
...
travelnet.remove_box = function(pos, oldnode, oldmetadata, digger)
	...
	if (not oldmetadata) or oldmetadata == "nil" or (not oldmetadata.fields) then
		minetest.chat_send_player(player_name, S("Error: Could not find information about the station that is to be removed."))
		return
	end
	...
	if (not owner_name)
		...
		or (not travelnet.targets[owner_name][station_network])
	then
		minetest.chat_send_player(player_name, S("Error: Could not find the station that is to be removed."))
		return
	end
	...
	-- inform the owner
	minetest.chat_send_player(owner_name, S("Station '@1' has been REMOVED from the network '@2'.",
		station_name, station_network))

	if digger and owner_name ~= player_name then
		minetest.chat_send_player(player_name, S("Station '@1' has been REMOVED from the network '@2'.",
			station_name, station_network))
	end
...
travelnet.can_dig_old = function(pos, player, description)
	...
	if creative and creative.is_enabled_for(name) then
		-- only a diamond pick can dig the travelnet
		if (not player:get_wielded_item())
			or player:get_wielded_item():get_name() ~= "default:pick_diamond"
		then
			return false
		end
	end
	...
	if minetest.check_player_privs(name, {travelnet_remove=true})
		or travelnet.allow_dig(name, owner, network_name)
	then
		return true
	end
	...
	elseif owner ~= name then
		minetest.chat_send_player(name, S("This @2 belongs to @2. You can't remove it.", description, owner))
		return false
	end
	return true
end
