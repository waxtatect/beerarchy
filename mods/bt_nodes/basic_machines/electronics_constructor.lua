...
local constructor_process = function(pos, player)
	...
	if player then
		...
		if owner then
			ranking.increase_rank(owner, "intelligence", 2)
		end
	end
end
...
local constructor_update_meta = function(pos)
	...
	meta:set_string("formspec",
		"size[8,10]"..
		"textlist[0,0;3,1.5;craft;"..table.concat(basic_machines.electronics_craft_recipe_order, ",")..
...
minetest.register_node("basic_machines:electronics_constructor", {
	...
	after_place_node = function(pos, placer)
		...
		meta:set_string("craft", "control logic unit")
