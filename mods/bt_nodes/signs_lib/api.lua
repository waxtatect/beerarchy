...
function get_sign_formspec(pos, nodename)
	...
	if minetest.registered_nodes[nodename].allow_widefont then
		...
		formspec[5] = "label[0.5,3.4;" .. S("Use wide font") .. "]"
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if fields.text and fields.ok then
		minetest.log("action", ("%s wrote \"%s\" to sign at %s")
			:format((playername or ""), fields.text:gsub('\\', '\\\\'):gsub("\n", "\\n"), pos_string
		))
	...
	elseif fields.on or fields.off then
		...
		if change then
			minetest.log("action", ("%s flipped the wide-font switch to \"%s\" at %s")
				:format((playername or ""), (fields.on and "off" or "on"), minetest.pos_to_string(pos)
			))
			signs_lib.update_sign(pos, fields)
