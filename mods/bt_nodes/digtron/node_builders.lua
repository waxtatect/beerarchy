...
minetest.register_node("digtron:builder", {
	...
	on_receive_fields = function(pos, formname, fields, sender)
		...
		if fields.set then
			...
			minetest.add_entity({x=buildpos.x, y=buildpos.y, z=z_pos}, "digtron:marker"):set_yaw(1.5708)
			if z_pos >= buildpos.z then
				minetest.add_entity({x=buildpos.x, y=buildpos.y, z=z_pos - period}, "digtron:marker"):set_yaw(1.5708)
			end
			if z_pos <= buildpos.z then
				minetest.add_entity({x=buildpos.x, y=buildpos.y, z=z_pos + period}, "digtron:marker"):set_yaw(1.5708)
			end
