...
minetest.register_node("digtron:intermittent_digger", {
	...
	damage_creatures = function(player, pos, controlling_coordinate)
		local facing = minetest.get_node(pos).param2
		local target_pos = digtron.find_new_pos(pos, facing)
		local meta = minetest.get_meta(pos)
		if (target_pos[controlling_coordinate] + meta:get_int("offset")) % meta:get_int("period") == 0 then
			digtron.damage_creatures(player, target_pos, 8)
		end
	end
})
...
minetest.register_node("digtron:intermittent_soft_digger", {
	...
	damage_creatures = function(player, pos, controlling_coordinate)
		...
		local target_pos = digtron.find_new_pos(pos, facing)
		if (target_pos[controlling_coordinate] + meta:get_int("offset")) % meta:get_int("period") == 0 then
			digtron.damage_creatures(player, target_pos, 4)
		end
	end,
})
