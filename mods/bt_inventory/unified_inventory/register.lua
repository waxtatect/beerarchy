...
local trash = minetest.create_detached_inventory("trash", {
	...
	on_put = function(inv, listname, index, stack, player)
		...
		minetest.sound_play("trash", {to_player=player_name, gain = 1.0}, true)
	end,
})
		...
		if minetest.check_player_privs(player_name, {home=true}) then
			...
			if home ~= nil then
				minetest.sound_play("dingdong",
						{to_player=player_name, gain = 1.0}, true)
	...
	action = function(player)
		...
		if minetest.check_player_privs(player_name, {home=true}) then
			if unified_inventory.go_home(player) then
				minetest.sound_play("teleport", {to_player = player_name}, true)
			end
		else
			minetest.chat_send_player(player_name,
				S("You don't have the \"home\" privilege!"))
...
unified_inventory.register_page("craft", {
	get_formspec = function(player, perplayer_formspec)
		...
		formspec = formspec.."listring[context;craft]"
