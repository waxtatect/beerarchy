...
local function add_ore(modname, description, mineral_name, oredef)
	...
	for tool_name, tooldef in pairs(oredef.tools) do
		...
		if tool_name == "sword" then
			...
			if description == "Silver" then
				tdef.range = 5
				tdef.light_source = 12
			end
		end
		...
		if tool_name == "pick" then
			...
			tdef.light_source = description == "Silver" and 12 or 0
		end
	...
	mithril = {
		...
		--]]
			...
			sword = {
				...
				choppy = {times = {[3] = 0.65}, uses = 200, maxlevel = 1},
			},
		},
...
function minetest.handle_node_drops(pos, drops, digger)
	...
	if wielded_item ~= "moreores:pick_silver" and wielded_item ~= "moreores:shovel_silver" then
		return old_handle_node_drops(pos, drops, digger)
	end

	local nn = minetest.get_node(pos).name

	local get_item_group = true
	if wielded_item == "moreores:pick_silver" then
		get_item_group = minetest.get_item_group(nn, "cracky") == 0
	elseif wielded_item == "moreores:shovel_silver" then
		get_item_group = minetest.get_item_group(nn, "soil") == 0
	end

	if get_item_group then
		return old_handle_node_drops(pos, drops, digger)
	end
