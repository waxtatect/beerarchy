...
local function iseagle(pos2)
	...
	for _, obj in ipairs(objs) do
		if not obj:is_player() then
			...
			if luaent and luaent.name and luaent.name == 'aerotest:eagle' then
				number = number + 1
			end
		end
	end
...
-- Add chat command to spawn an eagle
...
-- Show what is on eagle's menu today
...
minetest.register_chatcommand("rnd", {
	description = "Test random numgen",
	privs = {server = true},
	func = function(name)
		for i = 1, 100, 1 do
			minetest.chat_send_player(name, dump(aerotest.random()))
		end
	end
})