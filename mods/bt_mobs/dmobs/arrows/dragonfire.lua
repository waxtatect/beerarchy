...
function dmobs.register_fire(fname, texture, dmg, replace_node, explode, ice, variance, size)
	minetest.register_entity(fname, {
		...
		on_step = function(self, obj, pos)
			local remove = minetest.after(2, function() self.object:remove() end)
			local pos = self.object:get_pos()
			local objs = minetest.get_objects_inside_radius({x=pos.x,y=pos.y,z=pos.z}, 2)
			for k, obj in pairs(objs) do
				if obj:get_luaentity() then
					if obj:get_luaentity().name ~= fname and
						obj:get_luaentity().name ~= "dmobs:dragon_red" and
						obj:get_luaentity().name ~= "dmobs:dragon_blue" and
						obj:get_luaentity().name ~= "dmobs:dragon_black" and
						obj:get_luaentity().name ~= "dmobs:dragon_green" and
						obj:get_luaentity().name ~= "dmobs:dragon_great_tame" and
			...
			for dx = 0, 1 do
				for dy = 0, 1 do
					for dz = 0, 1 do
						...
						if n ~= fname and n ~= "default:dirt_with_grass" and
							n ~="default:dirt_with_dry_grass" and
							n ~="default:stone" and
							not minetest.is_protected(t)
						then
							if explode and minetest.registered_nodes[n].groups.flammable or
								ice and n == "default:water_source" --[[or math.random(1, 100) <= 1]]
							then
			...
			local apos = self.object and self.object:get_pos()
...
dmobs.register_fire("dmobs:lightning_plyr", "dmobs_lightning.png", 2, "tnt:boom", true, false, 0, 0.5)