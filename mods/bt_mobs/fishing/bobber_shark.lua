-----------------------------------------------------------------------------------------------
-- Fishing - crabman77's version - Bobber Shark
...
-- WHEN RIGHTCLICKING THE BOBBER THE FOLLOWING HAPPENS (CLICK AT THE RIGHT TIME WHILE HOLDING A FISHING POLE)
	on_rightclick = function (self, clicker)
		...
		if string.find(item_name, "fishing:pole_") then
			if player_name ~= self.owner then return end
			if self.prize then
				if math.random(1, 100) <= settings.escape_chance and not self.has_treasure then -- fish escaped
					if settings.message then
					...
					if self.has_treasure then
						func.notify(player_name, desc)
						self.has_treasure = false
					elseif settings.message then
		...
		elseif item_name == "fishing:baitball_shark" then
			...
			minetest.add_particlespawner({
				amount = 30,
				time = 0.5,
				minpos = {x = pos.x, y = pos.y - 0.0325, z = pos.z},
				maxpos = {x = pos.x, y = pos.y, z = pos.z},
				minvel = {x = -2, y = -0.0325, z = -2},
				maxvel = {x = 2, y = 3, z = 2},
...
-- AS SOON AS THE BOBBER IS PLACED IT WILL ACT LIKE
	on_step = function(self, dtime)
		...
		-- change item on line
		self.timer = 0
		if self.prize and settings.have_true_fish and prizes["true_fish"]["big"][self.prize[1]..":"..self.prize[2]] then
		...
		if settings.treasure_enable and chance == 1 then
			if math.random(1, 100) <= settings.treasure_chance then
				self.prize = prizes["treasure"][math.random(1, #prizes["treasure"])]
				self.has_treasure = true
			end
		elseif chance <= settings.shark_chance then
			if self.water_type and self.water_type == "sea" then
				self.prize = prizes["sea"]["big"][math.random(1, #prizes["sea"]["big"])]
			else
				self.prize = prizes["rivers"]["big"][math.random(1, #prizes["rivers"]["big"])]
			end
			...
			if settings.have_true_fish then
				...
				for _, obj in pairs(objs) do
					if obj:get_luaentity() then
						...
						if prizes["true_fish"]["big"][name] then
							self.prize = prizes["true_fish"]["big"][name]
							obj:remove()
