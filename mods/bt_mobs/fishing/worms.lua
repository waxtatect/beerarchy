...
minetest.register_craftitem("fishing:bait_worm", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		if not placer or not placer:is_player() then -- can happen, e.g. if placed using minetest.place_node
			return itemstack
		end
	...
	on_step = function(self, dtime)
		...
		local look_whats_up = function(self)
			...
			elseif minetest.get_item_group(n.name, "snappy") ~= 0 then
				self.object:move_to({
					x = pos.x + (0.001 * math.random(-32, 32)),
					y = pos.y - (0.001 * math.random(0, 64)),
					z = pos.z + (0.001 * math.random(-32, 32))
				})
			...
			elseif minetest.get_item_group(n.name, "soil") ~= 0 then
				...
				elseif self.object:get_hp() > 200 then -- leave dirt to see whats going on
					self.object:move_to({
						x = pos.x + (0.001 * math.random(-2, 2)),
						y = pos.y + 0.003,
						z = pos.z + (0.001 * math.random(-2, 2))
					})
				...
				elseif goal_04 ~= 0 or goal_4a ~= 0 then
					...
					self.object:move_to({
						x = pos.x + (0.001 * math.random(-8, 8)),
						y = pos.y,
						z = pos.z + (0.001 * math.random(-8, 8))
					})
				end
			end
		end
...
if not settings.new_worms_source then
	...
	local function hoe_on_use(itemstack, user, pointed_thing, uses)
		...
		if minetest.is_protected(upos, name) then
			minetest.record_protection_violation(upos, name)
			return
		end
		...
		if not func.creative(name) then
			local tool_name = itemstack:get_name()
			itemstack:add_wear(65535 / (uses - 1))
			if itemstack:get_wear() == 0 and minetest.get_modpath("invtweak") then
				local index = user:get_wield_index()
				minetest.sound_play("invtweak_tool_break", {pos = user:get_pos(), gain = 0.9, max_hear_distance = 5}, true)
